...
const SportSection: React.FC = () => {
  const { data, isLoading } = useGetAllPostWithTransformationResponseQuery({
    per_page: 1,
    orderby: "date",
    categories: 15, // ID catégorie sport
    _embed: true,
  });
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 md:pt-16">
      {/* En-tête avec éléments visuels sportifs */}
      <div className="flex items-center justify-between mb-10 relative">
        <div className="flex items-center space-x-4">
          <div className="border-l-8 pl-3 border-red-500">
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-1 max-w-lg">
              Compétitions, performances et événements sportifs qui font vibrer
              les passionnés.
            </p>
          </div>
        </div>
        ...
        <Link
          href="/category/sport"
          className="hidden md:flex items-center text-sm bg-red-600 hover:bg-red-700 text-white px-5 py-2 rounded-lg font-medium transition-all duration-300 group shadow-md hover:shadow-lg"
        >
          Voir tout
          ...
          {loading ? (
            ...
            recent?.slice(1, 4).map((post: WPBlogPost) => (
              <div
                ...
                key={post.id}
              >
                <BlogCard category="Sport" article={post} variant="compact" />
              </div>
            ))
          )}
        </div>
